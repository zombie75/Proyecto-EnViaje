...
<head>
	<meta charset="UTF-8">
	<title>Servicios de comida</title>
	<link rel="stylesheet" href="css/estilos_comidas.css">
</head>
<body>
		<div id="todo">
				<div id="top">
			...
			<div id="centro">
				...
				$rgn = $_GET["region"];
				$res = $mysqli->query("SELECT * FROM servicio_comida ORDER BY id_servicio_comida DESC ");
				...
				$nf2 = $f2[0]; //nro filas
				for ($i=$nf; $i>=0 ; $i--) { 
					$resi = $mysqli->query("SELECT * FROM servicio_comida WHERE id_servicio_comida='$i' ");
					$fi = mysqli_fetch_row($resi);
					if (strcmp($fi[0], "") == 0) {
						//comprueba que no este vacio
					}else{
						$resi2 = $mysqli->query("SELECT * FROM foto WHERE id_servicio_comida='$i' ");
						$fi2 = mysqli_fetch_row($resi2);
						if ($rgn == 0) {
							echo "<a href='comidasver.php?id_servicio_comida=".$fi[0]."' style='text-decoration:none;color:black'><div id=\"comida\">
						";//inicio div comida individual con variables por url, ademas style que cambia color de letras y subrayado de link

							echo "<div id='datos'>";
							echo "<br><div id=\"nombre\">".$fi[2]."</div>";//nombre
							...
							echo "<div id=\"direccion\"> ".$fi[3]."</div>";//direccion
							echo "<div id=\"desc\"> ".substr($fi[14], 0, 50)."...</div></div>";//descripcion acordada a 50 caracteres
							echo '<div id="delaimg"><div id=\"imag\"><img src="data:image/jpeg;base64,'.base64_encode($fi2[1]).' " width="80%" height="80%"/></div>
							</div></div></a>
						';//imagen y cierre de div comida individual
					}else{
						if (strcmp($rgn, $fi[11])==0) {
						...
						";//inicio div comida individual con variables por url, ademas style que cambia color de letras y subrayado de link
							...
							echo "<br><div id=\"direccion\"> ".$fi[3]."</div>";//direccion
							echo "<br><div id=\"desc\"> ".substr($fi[14], 0, 50)."...<br></div>";//descripcion acordada a 50 caracteres
