...
<html lang="en">
	<head>
		...
		<link rel="stylesheet" href="css/estilo_hospedajes.css">
	</head>
	...
	<body>
		<div id="todo">
			...
			<div id="centro">
				...
				echo '<form action="hospedajes.php?region=region">
				...
				$rgn = $_GET["region"];
				$res = $mysqli->query("SELECT * FROM servicio_hospedaje ORDER BY id_servicio_hospedaje DESC ");
				...
				for ($i=$nf; $i>=0 ; $i--) { 
					...
					if (strcmp($fi[0], "") == 0) {
						...
						if ($rgn == 0) {
							echo "<a href='hospedajesver.php?id_servicio_hospedaje=".$fi[0]."' style='text-decoration:none;color:black'><div id=\"hospedaje\">
						";//inicio div hospedaje individual con variables por url, ademas style que cambia color de letras y subrayado de link
							...
							echo "<div id=\"direccion\"> ".$fi[8]."</div>";//direccion
							echo "<div id=\"desc\"> ".substr($fi[12], 0, 50)."...</div>";//descripcion acordada a 50 caracteres
							echo '</div><div id=delaimg> <div id=\"imag\"> <img src="data:image/jpeg;base64,'.base64_encode($fi2[1]).' " width="85%" height="85%"/> </div>
							</div></div></a>
						';//imagen y cierre de div picada individual
					}else{
						if (strcmp($rgn, $fi[9])==0) {
								echo "<a href='hospedajesver.php?id_servicio_hospedaje=".$fi[0]."' style='text-decoration:none;color:black'><div id=\"hospedaje\">
						";//inicio div hospedaje individual con variables por url, ademas style que cambia color de letras y subrayado de link
